...
class ContinualLearner(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        self.prev_W = None      #-> W for storing previous W
    ...
    def estimate_fisher(self, dataset, allowed_classes=None, collate_fn=None):
        ...
        mode = self.training
        if self.cls_type == 'lstm' and self._is_on_cuda():
            print("lstm and cuda")
            self.lstm_input_dropout.eval()
            drop = self.lstm.dropout
        ...
        self.train(mode=mode)
        if self.cls_type == 'lstm' and self._is_on_cuda():
            print("lstm and cuda reset")
            self.lstm.dropout = drop
