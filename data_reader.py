...
class data_reader:
    ...
    def save_data(self):
        np.save('data/opportunity.npy', self.data)
        np.save('data/opportunity.classes.npy', self.id2label)

    # def save_data(self):
    #     with open('data/opportunity.pkl', 'wb') as f:
    #         pickle.dump(self.data, f)
    #     with open('data/opportunity.classes.pkl', 'wb') as f:
    #         pickle.dump(self.id2label, f)

    # def save_data(self):
    #     f = h5py.File('data/opportunity.h5')
    #     for key in self.data:
    #         f.create_group(key)
    #         for field in self.data[key]:
    #             f[key].create_dataset(field, data=self.data[key][field])
    #     f.close()
    #     with open('data/opportunity.h5.classes.json', 'w') as f:
